...
#komut basinda kullanilacak karakter
...
async def new_project(ctx):
    ...
    await ctx.send("aciklama girin")
    description=await bot.wait_for('message',check=check)
    data.append(description.content)

    yapimcilar = [x[0] for x in manager.get_yapimci()]
    await ctx.send("Lütfen projenin yapimcisini girin!", delete_after=60.0)
    await ctx.send("\n".join(yapimcilar), delete_after=60.0)
