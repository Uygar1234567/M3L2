...
statuses = [(_,) for _ in (
    ['Prototip Oluşturma', 'Geliştirme Aşamasında', 'Tamamlandı, kullanıma hazır', 'Güncellendi', 'Tamamlandı, ancak bakımı yapılmadı']
)]
yapimcilar = [(_,) for _ in (['Uygar', 'Umut', 'Tugrul', 'Jankat'])]
...
class DB_Manager:
    ...
    def create_tables(self):
        ...
        with conn:
            ...
            conn.execute('''CREATE TABLE IF NOT EXISTS yapimci (
                yapimci_id INTEGER PRIMARY KEY,
                yapimci_name TEXT
            )''')
